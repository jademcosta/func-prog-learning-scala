...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase().groupBy(letter => letter).map {elem => (elem._1, elem._2.size)}.toList.sortBy(_._1)
  //TODO: filter: .filter(_._1.isLetter) ?
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(List())
    case (x :: xs) => {
      val tailCombinantions = combinations(xs)
      tailCombinantions ++ (
        for {
	      i <- (1 to x._2)
	      j <- tailCombinantions
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (for ( (ch, qtd) <- x ) yield {
      y.filter( _._1 == ch ).headOption match {
        case Some((_, ys)) => (ch, qtd - ys)
        case None => (ch, qtd)
      }
    }).filter(_._2 > 0)
  }
    ...
    def loop(occurrences: Occurrences): List[Sentence] = {
      if(occurrences.isEmpty)
        ...
        for {
	      ...
	      sentenceAnagram <- loop(subtract(occurrences, combinations))
        } yield words :: sentenceAnagram
    }
