...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("Empty.mostRetweeted called")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    val accumulator = if (elem.retweets > acc.retweets) elem else acc
    right.mostRetweetedAcc(right.mostRetweetedAcc(accumulator))
  }
  
  override def descendingByRetweet: TweetList = {
    val setWithoutMostRetweeted = (left remove mostRetweeted) union (right remove mostRetweeted)
