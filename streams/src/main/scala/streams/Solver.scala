...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    (for { 
      (newBlock, movement) <- b.legalNeighbors 
    } yield (newBlock, movement :: history)).toStream
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty) initial
	    ...
	    val newPaths = for {
	      position <- initial
	      newNeighbors <- newNeighborsOnly(neighborsWithHistory(position._1, position._2), explored)
	    } yield newNeighbors
	    
	    initial ++: from(newPaths, explored ++ (initial.map(_._1)).toSet)
    }
  }
